...
1. **Open an issue describing your intent** — what you want to change, why, and how it
   affects (or provably does not affect) the stated guarantees. Cite the RFC section
...
contribution there is.** Contradictions between the RFC text, the JSON Schemas, and the
fixtures; ambiguities where two independent implementations could reasonably disagree
(see the changelog's CLARIFIED items for the shape); fixtures — worked policies or
...
jointly through the change-set process below, and you can author the change set.
