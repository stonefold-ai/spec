# Registry (Domain Model) — Specification v0.3
...
**Status:** v0.3 (alongside Stele v0.3). **Foundational layer** — read alongside the SIF specification ([`00`](00-sif-wire-format.md)); Stele ([`01`](01-stele-policy-language.md)) and the policies reference the names declared here.
...
obligationRegistries:{ … }   # systems of record requireMatch matches against (§5b)
...
preconditionChecks:                         # named checks — bare or object form (v0.3)
...
## 5e. Sources a control reads (v0.3)

Some controls depend on content that **ages** or can become **unreachable**: a
critical-analyte list, a sanctions list, a tariff table, a state only another system
can report. Declare those by name, so a gate can say which one it depends on:
...
sources:
  ...
  supplierAccountChangedAt:
    kind: fact
    derivedFrom: [Supplier.updateBankAccount]
```

`kind` is `ruleSet` (content maintained outside the gateway), `record` (something in
another system), or `fact` (something the deployment maintains). `derivedFrom` names
the actions a fact is maintained from — documentation today, and the anchor for a
later rule: a fact derived from an action the gateway never sees is a fact it cannot
maintain.

Each declared source has a **registered adapter** in gateway code, one method
answering *when was this content last known good* (`None` for undated). The
registration is code rather than YAML for the same reason a connector's
scope-reassertion capability is: it is a property of the implementation and is
reviewed with it. A declared source with no adapter resolves *unavailable* — never
fresh.

What reads this declaration is a gate's `reads:` (Stele §7.6), and the linter refuses
a `reads:` naming a source that is not declared here (Stele §13.22).

---

## 5d. Item-bearing actions (v0.3)

An action that carries a **list of targets in one field** — `markManyReviewed(itemIds)`,
`payBatch(payments)`, `closeAll(ticketIds)` — may declare it, so the gateway can decide
the items rather than only the call:
...
WorklistItem:
  actions:
    markManyReviewed:
      ...
      items:
        field: itemIds        # the data field carrying the items
        independent: true     # applying a SUBSET of them is meaningful
        maxItems: 500         # refuse a larger call outright
        key: invoiceNo        # where items are objects: the field that names one
```

`field` MUST name a field the action declares in `data` (the linter refuses a
mismatch — Stele §13.21, because a typo would silently stop the action fanning out and
leave every per-item gate inert).

**`independent` is the judgment call, and it is a safety property.** It asserts that
applying some items without the others is a meaningful outcome — that item 3 does not
depend on item 7. True of a worklist and a payment run. **False of both legs of a
transfer**, where applying one leg is worse than applying neither, and false of any set
the managed system treats as one transaction. Default `false`: declaring the items
without it changes nothing, which is the safe direction for a declaration whose author
has not thought about it yet.

`maxItems` bounds the cost. Per-item evaluation is N gate evaluations, and a call
carrying fifty thousand items is fifty thousand — so a deployment declares the size it
is willing to evaluate and the gateway refuses a larger call rather than working through
it quietly. The linter warns when a fanning-out action declares no ceiling.

What reads this declaration is Stele §12's per-item decision semantics.

---

## 5c. Closure dispositions (v0.3)
...
- **`capability`** — how `consume` composes with the effect's settlement, mirroring the connector scope-reassertion capabilities: **`transactional`** — the adapter can consume inside the same transaction as the effect's commit and the audit write (no consumed-without-effect, no effect-without-consumed); **`window`** — it cannot; consumption runs immediately after connector confirmation and the adapter's declared residual window is surfaced in the audit record (priced, not hidden). Unlike the connector's declared capability, this one is registry YAML: which guarantee a system of record offers is a reviewable fact about the domain, not about gateway code.
...
1. **Prefer the stock factories.** The common shapes ship pre-written and pre-verified in `stonefold_gates.stock` — `resource_state_in` (state membership), `cooling_off_elapsed` (the new-payee pattern, specification §14.1), `data_field_present` (explanation-required). Each is pure, deterministic, and **fails closed** (missing field / unparsable value / absent injected clock ⇒ `False`, never an exception). Most deployments should write no bespoke check code at all.
...
**`deny` names must exist too (Stele §13.1).** v0.2 carved out an exception letting `deny` reference undeclared actions ("forbid it before it exists"). It is removed: a deny of an unknown name adds no protection — default-deny already refuses anything undeclared — and is almost always a typo that would silently become a no-op. **You deny things that exist**: to pre-forbid a capability, declare the action in the registry and deny it in the policy (the pattern the worked registries use — `exportData` exists precisely so the payments policy can deny it). Adding a dangerous action to the registry then surfaces every policy that must be reviewed, instead of silently activating alongside a stale deny.

Worked registries: [`../examples/payments.registry.yaml`](../examples/payments.registry.yaml) (the demo domain) and [`../examples/worklist.registry.yaml`](../examples/worklist.registry.yaml). Each pairs with the policy of the same name.
