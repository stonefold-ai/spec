...
**Status:** v0.6 — change set v0.5 → v0.6 **ACCEPTED** 2026-07-09 ([`docs/RFC-changeset-v0.5-to-v0.6.md`](RFC-changeset-v0.5-to-v0.6.md)): the reference implements every item (CS-026…CS-039) and the TCK certifies the four new profiles (`hold-precondition`, `feedback`, `match`, `consume`), in-process and over the wire binding. Change set v0.6 → v0.6.1 is **OPEN** ([`docs/RFC-changeset-v0.6-to-v0.6.1.md`](RFC-changeset-v0.6-to-v0.6.1.md)); its items are mirrored below and normative on conflict. **Authors:** the agent-platform team.
