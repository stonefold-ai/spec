...
- **requireMatch** *(v0.3)* — the intent must match exactly one open obligation in a declared registry, which it consumes on settlement. *Ex:* `pay` only against an open purchase-order line — the same line can't pay two invoices.
...
- **source** *(v0.3)* — something a control reads that may age or become unreachable: a rule set, a record in another system, a fact the deployment maintains. Declared by name in the registry (`sources`, docs/06 §5e) with an adapter in gateway code that reports how old its content is. *Ex:* the list of analytes needing same-day review.
- **reads** *(v0.3)* — a gate's declaration of which sources it depends on, optionally with a maximum age and what to do when the source is older than that. Makes the dependency data rather than a comment, so it can be inspected without running the system. *(Stele `01` §7.6.)*
- **item-bearing action** *(v0.3)* — one action carrying a list of targets in a single field (`markManyReviewed(itemIds)`). Declared per action in the registry (`items`, docs/06 §5d); where the items are declared **independent**, each is decided on its own and the allowed ones are applied together. *Ex:* twenty worklist items in one call, seven closed and thirteen refused, instead of the whole call refused.
- **per-item verdict** *(v0.3)* — one outcome per item inside such a call, each with its own reason code, retry class and (for a held item) its own ticket. The call's own `decision` is `allow` only if every item was applied. *(Stele `01` §12.)*
- **standard check** *(v0.3)* — a precondition check whose name is reserved and whose semantics are normative, so a reviewer reads the behaviour from the spec rather than from the integrator's code. One exists: `dispositionIsDeclared`. *(Stele `01` §7.6.)*
- **disposition** *(v0.3)* — the declared outcome an actor states when it closes a unit of work: resolved, escalated, referred, duplicate. Declared per action in the registry (`closure`, docs/06 §5c); the subset that asserts the work was *done* is `claimsCompletion`. *Ex:* an agent may close a worklist item as `escalated` after the gateway refused its attempt to act, but not as `resolved`.
- **precondition check** — a deterministic test before an action. *In:* context → *out:* pass/fail — or hold, if declared `holdCapable` (v0.3). *Ex:* `payeeCoolingOffElapsed`.
